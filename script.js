...
document.addEventListener('DOMContentLoaded', function () {
  ...
  startButton.addEventListener('click', function () {
      welcomeScreen.classList.add('hidden'); // Добавляем класс 'hidden' для запуска анимации
  });
});
...
const scrollButton1 = document.getElementById('scroll-button1');
...
}, { threshold: 0.5 }); // 0.5 значит, что хотя бы 50% элемента должны быть видимы, чтобы сработал колбэк
...
function updateScroll() {
  const newPosition = (currentPage - 1) * 50;
...
function stopVideo() {
  myVideo.pause();
  myVideo.currentTime = 0; // Сбросить текущее время в начало
}
